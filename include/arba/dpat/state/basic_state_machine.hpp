...
public:
    ...
    using abstract_state_siptr = itru::shared_intrusive_ptr<abstract_state>;
...
protected:
    template <typename CallbackType, typename... Args>
        requires requires(abstract_state* ptr, CallbackType callback, Args&&... args) {
            { (ptr->*callback)(std::forward<Args>(args)...) };
        } && (!std::is_void_v<std::invoke_result_t<CallbackType, abstract_state, Args && ...>>)
