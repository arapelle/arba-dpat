...
{
public:
    explicit first_state(bool& valid);

    virtual ~first_state() override { *pvalid_ = false; }

    virtual int& execute(state_machine& stm, int& dest, const int& src, int offset) override
    {
        offset += offset;
        dest = src + offset;
        stm.set_state(nullptr);
        return dest;
    }
...
TEST(state_machine_tests, use_case__light_state__ok)
{
    ...
    abstract_state_siptr gen_int =
        itru::make_shared_intrusive_ptr<ut::first_state>(std::ref(valid));
    ...
    {
//        stm.manage_event('A');
        pdest = &stm.execute(dest, src, offset);
    }

    ASSERT_EQ(pdest, &dest);
    ASSERT_EQ(dest, 2042);
    ASSERT_EQ(src, 42);
