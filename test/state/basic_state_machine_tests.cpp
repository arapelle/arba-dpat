...
public:
    ...
    virtual abstract_state_siptr run(int& dest, const int& src, int offset) = 0;
};
...
public:
    ...
    virtual abstract_state_siptr run(int& dest, const int& src, int offset) override
    {
        offset += offset;
        dest = src + offset;
...
public:
    ...
    virtual abstract_state_siptr run(int&, const int&, int) override { return abstract_state_siptr(); }
...
public:
    ...
    {
        this->base_::invoke(&abstract_state::run, std::forward<Args>(args)...);
    }
